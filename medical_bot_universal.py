# medical_bot_universal.py
"""MEDICAL CHATBOT - UNIVERSAL LOGIN FOR ALL USERS"""
...
app = FastAPI(title="Medical Chatbot", version="5.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

SECRET_KEY = os.getenv('SECRET_KEY', 'medical-secret-key-2026-make-it-32-chars-long-here')
ALGORITHM = "HS256"
...
def verify_password(password: str, hashed: str) -> bool:
    """Verify password against hash - WORKS FOR ALL USERS"""
    ...
    try:
        ...
        result = bcrypt.checkpw(password_str.encode('utf-8'), hashed_str.encode('utf-8'))
        return result
...
async def register(user: UserRegister):
    try:
        ...
        print(f"  Password hash: {hashed_pw[:30]}...")
...
async def login(user: UserLogin):
    try:
        ...
        print(f"  Email: {db_user.get('email', 'N/A')}")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("🏥 MEDICAL CHATBOT - UNIVERSAL LOGIN")
    print("="*60)
    print("Server: http://localhost:8000")
    print("Login works for ALL registered users")
    print("="*60 + "\n")
